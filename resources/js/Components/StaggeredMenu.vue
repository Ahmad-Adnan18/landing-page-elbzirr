<template>
  <div class="w-full h-full sm-scope">
    ...
    >
      ...
      >
        <div class="flex items-center pointer-events-auto select-none" aria-label="Logo">
        </div>

        <button
          ref="toggleBtnRef"
          class="inline-flex justify-center items-center relative gap-[0.3rem] bg-transparent border-0 overflow-visible font-medium text-[#e9e9ef] leading-none cursor-pointer pointer-events-auto sm-toggle"
      ...
      >
        <div class="flex flex-col flex-1 gap-5 sm-panel-inner">
          ...
          >
            ...
            >
              <Link
                class="inline-block relative pr-[1.4em] font-semibold text-[2.5rem] sm:text-[4rem] text-black no-underline uppercase leading-none tracking-[-2px] transition-[background,color] duration-150 ease-linear cursor-pointer sm-panel-item pointer-events-auto"
                :href="item.link"
...
const handleItemClick = (item: StaggeredMenuItem, event: MouseEvent) => {
  // Close the menu
  toggleMenu();

  // If it's an anchor link on the same page, we might want to handle smooth scroll manually
  // But Inertia's Link with a hash usually handles it.
  // However, to be safe and ensure the menu closing animation finishes or doesn't conflict:
  ...
  if (item.link.includes('#')) {
      ...
      if (targetEl) {
          // We are on the page, scroll to it
          // event.preventDefault(); // Optional: if we want to handle scroll manually
          // But since we are using <Link>, it might navigate.
          // Let's just let Inertia handle the navigation/hash change.
          // But we can ensure smooth scroll if needed.
      }
  }
};
